...
constexpr double	ARM_JNT1_VISCOUS = 60.0;		// 粘性係数	//XY平面上で実験字は　1.0
constexpr double	ARM_JNT2_VISCOUS = 60.0;		// 粘性係数
...
//	ロボットの初期姿勢
double	init_pos[2] = { 4 * PI / 4.0 - PI / 6.0 * finger_state ,+PI / 3.0 * finger_state };	// ロボット初期姿勢
double	init_posF2[2] = { 4 * PI / 4.0 + PI / 6.0 * finger_state, -PI / 3.0 * finger_state };	// ロボット初期姿勢
...
struct  Impedance {
	...
	Impedance() {
		...
		for (int i = 0; i < 3; i++) {
			for (int k = 0; k < 3; k++) {
				matInit(&dqkoTi[i][k], 4, 4);
				for (int l = 0; l < 3; l++) {
					matInit(&dqjdqk0Ti[i][k][l], 4, 4);
				}
			}
		}
	}
