...
#define	ARM_SENSOR_LENGTH	0.0001
	...
	// 図形移動
	glTranslated(_this->getFinger()->save_jnt_pos[count][ARM_M1], 0.0, 0.0);	// 仮に1軸
	// アーム
...
#define	OBJ_LENGTH	0.15
	...
	glPushMatrix();
	// 土台
//	glTranslated(_this->save_obj_pos[count], ARM_BASE_Y/2.0, 0.0);	// 1軸
	glTranslated(_this->getFinger()->save_obj_pos[count][CRD_X], _this->getFinger()->save_obj_pos[count][CRD_Z], _this->getFinger()->save_obj_pos[count][CRD_Y]);	// 1軸
